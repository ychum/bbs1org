...
function us_result_page(string $title, array $changes, string $error = ''): void
{
    ...
    if ($error !== '') {
        ...
        foreach ($changes as $change) $body .= '<li>' . us_h($change) . '</li>';
        $body .= '</ul>';
    } else {
...
function us_update_page(?array $release = null, string $error = ''): void
{
    ...
    $body = '<h1 class="update-title">系统升级</h1><p class="update-sub">检测并安装 ' . us_h(UPDATE_REPOSITORY) . ' 主分支的最新代码，也可单独同步当前代码对应的数据库结构。</p>';
    ...
    if ($release) {
        ...
        if ($changes) {
            ...
            foreach ($changes as $change) {
                ...
                $body .= '<li><label><input type="checkbox" name="files[]" value="' . us_h($path) . '" form="online-update-form" checked> ' . us_h((string)($change['type'] ?? '变更')) . '：' . us_h($path) . '</label></li>';
            }
            $body .= '<li><label><input type="checkbox" name="sync_schema" value="1" form="online-update-form" checked> 文件更新后同步数据库结构</label></li>';
            $body .= '</ul>';
        } else {
